...
public class ProgressBarController : MonoBehaviour {
	...
	void Update () {
		...
		float prop = ((float)weight - (float)global.Global.minWeight) / ((float)global.Global.maxWeight - (float)global.Global.minWeight);
	...
	void FixedUpdate()
    {
        ...
        Debug.Log(string.Format("{0},{1}", targetPosX, pointer.transform.localPosition.x));
